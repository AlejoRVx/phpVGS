...
<section id="catalogo-consolas" class="py-6">
    <h2 class="text-2xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400 mb-6 sm:mb-8 text-center">
        🎮 Catálogo de Consolas Destacadas
    </h2>
    ...
    @if($productos->isEmpty())
        <div class="flex flex-col items-center justify-center py-20 bg-gray-800/30 rounded-3xl border-2 border-dashed border-purple-900/50">
            ...
            <h3 class="text-2xl sm:text-3xl font-bold text-white mb-2">¡Vaya! No encontramos esa consola</h3>
            <p class="text-gray-400 text-center max-w-md mb-8 px-6">
                {{ request('q')
                    ...
                    : 'Parece que aún no tenemos consolas disponibles.'
                }}
            </p>
            @if(request('q'))
                <a href="{{ route('productos.consolas') }}" class="px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold rounded-full transition transform hover:scale-105 shadow-lg shadow-purple-600/30">
    ...
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            @foreach ($productos as $producto)
                <div class="bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-2xl overflow-hidden transition duration-300 transform hover:scale-[1.02] hover:shadow-blue-500/30 border border-gray-700 flex flex-col">
                    ...
                    <div class="p-5 sm:p-6 flex flex-col flex-grow">
                        <a href="{{ route('productos.resenas', ['id' => $producto->id]) }}">
                            <h3 class="text-lg sm:text-xl font-bold text-blue-400 mb-2 hover:text-blue-300 transition-colors">{{ $producto->nombre }}</h3>
                        </a>
                        ...
                        <div class="flex justify-between items-center mt-auto pt-4 border-t border-gray-700">
                            ...
                            @auth
                                <form class="add-to-cart-form" data-product-id="{{ $producto->id }}" method="POST">
                                    @csrf
                                    <button type="submit" class="px-4 sm:px-5 py-2 bg-purple-600 text-white font-semibold rounded-lg transition duration-300 hover:bg-purple-500 shadow-lg shadow-purple-600/50 text-sm">
                                        Añadir 🛒
                                    </button>
                                </form>
                            @endauth
