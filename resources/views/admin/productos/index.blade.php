...
<section class="mb-8 sm:mb-12">
    <form method="GET" action="{{ route('admin.productos.search') }}" class="max-w-3xl mx-auto flex">
        <input type="search" name="q" value="{{ request('q') }}" placeholder="Buscar producto..."
            class="w-full px-5 py-3.5 text-white bg-gray-900 rounded-l-lg border-2 border-r-0 border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-base">
        <button type="submit" class="px-6 sm:px-8 py-3.5 bg-purple-800 text-white rounded-r-lg hover:bg-purple-600 transition duration-300 font-semibold">
            Buscar
        </button>
    </form>
</section>
...
<div class="flex flex-wrap justify-center gap-2 sm:gap-4 mb-6">
    <a href="{{ route('admin.productos.index') }}"
       class="px-3 sm:px-4 py-2 rounded transition text-sm sm:text-base {{ !request('tipo') ? 'bg-blue-600 text-white' : 'bg-gray-700 hover:bg-gray-600' }}">Todos</a>
    <a href="{{ route('admin.productos.index', ['tipo' => 'Juego']) }}"
       class="px-3 sm:px-4 py-2 rounded transition text-sm sm:text-base {{ request('tipo') === 'Juego' ? 'bg-blue-600 text-white' : 'bg-gray-700 hover:bg-gray-600' }}">Juegos</a>
    <a href="{{ route('admin.productos.index', ['tipo' => 'Consola']) }}"
       class="px-3 sm:px-4 py-2 rounded transition text-sm sm:text-base {{ request('tipo') === 'Consola' ? 'bg-purple-600 text-white' : 'bg-gray-700 hover:bg-gray-600' }}">Consolas</a>
</div>
